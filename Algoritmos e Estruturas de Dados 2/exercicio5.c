...
void printaLista (Lista *a){
	...
	for (rastreador = a->raiz; rastreador != NULL; rastreador = rastreador->prox){
		if (rastreador->chave >nivel) printf ("\nNivel %d: ",nivel=rastreador->chave);
		printf ("%c",rastreador->dado.dado);
	}
...
/*
void printaString (char *string, int inicio, int fim){
	int i;
	for (i=inicio; i <fim; i++)printf ("%c",string[i]);
	printf ("\n");
	return;
}
*/

void expressaoParaArvore (Ponteiro *p, char *string, int inicio, int fim){
	int i, parenteses=0, parentesesfinal, parentesesinicial;
	Dado d;
	Ponteiro no;
	
	//printaString (string, inicio, fim);
	
	if (inicio==fim) return;
	else if (fim-inicio == 1){ //caso pra folha
		d.dado = string[inicio];
		(*p) = criaNo((int)d.dado, d);
		return;
	}
	
	for (i=fim-1;i>=inicio;i--){//varre procurando + ou - da direita pra esquerda
		if (string[i]==')') parenteses--;
		else if (string[i]=='(') parenteses++;
		if ((string[i] == '+' || string[i] == '-') && parenteses==0){
			d.dado = string[i];
			(*p) = criaNo((int)d.dado, d);
			expressaoParaArvore(&((*p)->filho[esq]), string, inicio, i);
			expressaoParaArvore(&((*p)->filho[dir]), string, i+1, fim);
			return;
		}
	}
	...
	for (i=fim-1;i>=inicio;i--){//varre procurando ) da direita pra esquerda
		if (string[i]==')'){
			parentesesfinal = i;
			break;
		}
	...
	for (i=inicio;i<fim;i++){//varre procurando ( da esquerda pra direita
		if (string[i]=='('){
			parentesesinicial = i;
			break;
		}
	...
	return expressaoParaArvore(p, string, parentesesinicial+1, parentesesfinal);	
}

int main (){
	int i,j, n, casos, controledelinha=0;
	char string[102];
	Arvore a;
	Dado d;
	Lista l;
	...
	while (scanf ("%c",&string[i]) != EOF){
		if (string[i]==' ') continue;
			...
			if (controledelinha==1)printf ("\n\n");
			n=i;
			i=0;
			
			expressaoParaArvore (&(a.raiz), string, i, n);
