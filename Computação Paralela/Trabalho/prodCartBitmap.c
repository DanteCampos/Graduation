...
void printBitmapUpper (char * P, unsigned long long vertices_P){
...
int main(){
  ...
  for (dirIndex=0; dirIndex < 3; dirIndex++){ 
    ...
    while (1) {
      ...
      bitmap_P = bitmap_create(vertices_P);
      ...
      // ------------------------------------------------------------------
      cartProdBitmapG (edge_array_G, bitmap_P, edges_G, vertices_H, vertices_P);
      cartProdBitmapH (edge_array_H, bitmap_P, edges_H, vertices_G, vertices_P);
      // ------------------------------------------------------------------

      // Calculating passed time
      delta_time = clock() - delta_time;
      printf("Finished!\n");
      // printBitmapUpper(bitmap_P, vertices_P);
