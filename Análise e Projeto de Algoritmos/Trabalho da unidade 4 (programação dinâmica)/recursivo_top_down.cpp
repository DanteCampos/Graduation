...
int dpSol(int i, int j){
	...
	int seg;
	for (seg = 0; i+seg < s1_size and j + seg < s2_size and S1[i+seg] == S2[j+seg]; seg++);
	...
	for (; i+l-1 < s1_size and j+l-1 < s2_size; l++); 
		ans = max({dpSol(i + 1, j), dpSol(i, j + 1), ans});

	/*
	for(int l = k; i + l - 1 < s1_size and j + l - 1 < s2_size; l++){
